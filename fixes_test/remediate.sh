...
content here >> "$EVID_LOG" 2>&1 || true
...
if [ $? -eq 0 ]; then
  ...
  content here >> "$EVID_LOG" 2>&1 && echo "  ✔ Evidence: Check now PASSES" | tee -a "$LOG_FILE" || echo "  ! Evidence: Manual check required" | tee -a "$LOG_FILE"
