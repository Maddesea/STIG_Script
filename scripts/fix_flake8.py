...
# history/__init__.py
replace_in_file("stig_assessor/history/__init__.py", "from .models import Hist\nfrom .manager import HistMgr\n\n__all__ = [\"Hist\", \"HistMgr\"]\n\nfrom .models import Hist\nfrom .manager import HistMgr", "from .models import Hist\nfrom .manager import HistMgr\n\n__all__ = [\"Hist\", \"HistMgr\"]")

# history/manager.py
replace_in_file("stig_assessor/history/manager.py", "from contextlib import suppress\n", "")
...
text = re.sub(r"from datetime import datetime\n.*from datetime import datetime", "from datetime import datetime", text, flags=re.DOTALL)
text = re.sub(r"import xml.etree.ElementTree as ET\n.*import xml.etree.ElementTree as ET", "import xml.etree.ElementTree as ET", text, flags=re.DOTALL)
...
replace_in_file("stig_assessor/remediation/extractor.py", "        from stig_assessor.exceptions import ParseError\n        raise ParseError", "        raise ParseError")
with open("stig_assessor/remediation/extractor.py", "r") as f:
    text = f.read()
if "from stig_assessor.exceptions import ParseError" not in text:
    text = text.replace("from stig_assessor.exceptions import ValidationError", "from stig_assessor.exceptions import ValidationError, ParseError")
...
# ui/cli.py
replace_in_file("stig_assessor/ui/cli.py", "f\"STIG Assessor CLI (\"", "\"STIG Assessor CLI (\"")
replace_in_file("stig_assessor/ui/cli.py", "f\"  No actionable fixes extracted.\"", "\"  No actionable fixes extracted.\"")
