...
def temp_ckl(tmp_path):
    ckl_path = tmp_path / "test_poam_bulk.ckl"
...
def test_export_poam(temp_ckl):
    ...
    assert "Control Number,Vulnerability Description,Severity,Status,Comments,Checklist Name" in csv_str
...
def test_bulk_edit_regex(temp_ckl, tmp_path):
    ...
    assert v1_status == Status.NOT_APPLICABLE
    assert v1_comment == "comment 1\nRegex match"
