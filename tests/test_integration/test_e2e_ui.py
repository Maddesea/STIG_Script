...
class TestE2EWorkflows(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.results_json_path.write_text('{"results": [{"vid": "V-999999", "ok": true, "msg": "Fix applied."}]}')
    ...
    def test_01_cli_create(self, mock_stderr):
        ...
        args = ["--create", "--xccdf", str(self.xccdf_path), "--asset", "E2E_SERVER", "--out", str(out_ckl)]
    ...
    def test_02_cli_apply_results(self, mock_stderr):
        ...
        args = [
            "--apply-results", str(self.results_json_path),
            "--checklist", str(self.base_ckl_path),
            "--results-out", str(out_updated)
        ]
        result = cli_main(args)
    ...
    def test_03_cli_merge(self, mock_stderr):
        ...
        args = [
            "--merge", 
            "--base", str(self.base_ckl_path),
            "--histories", str(self.hist_ckl_path),
            "--merge-out", str(out_merged)
        ]
        result = cli_main(args)
    ...
    def test_04_cli_smart_defaults(self, mock_stderr):
        ...
        self.assertEqual(result, 0)
        expected_out = self.xccdf_path.with_name(f"SMART_TEST_{self.xccdf_path.stem}.ckl")
    ...
    def test_05_gui_workflow(self, mock_err, mock_info):
        ...
        self.assertTrue(any(l for l, _ in gui._inline_labels if "Missing input" in l.cget("text")))
