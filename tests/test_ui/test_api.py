...
from stig_assessor.ui.web.api import (
    handle_ping,
    handle_stats,
    route_request,
    _decode_to_temp,
...
class TestWebAPI(unittest.TestCase):
    ...
    def setUp(self):
        self.b64_empty_ckl = base64.b64encode(b"<CHECKLIST></CHECKLIST>").decode("utf-8")
    ...
    def test_handle_extract_fixes(self, mock_Extractor, mock_make_archive, mock_encode):
        ...
        mock_make_archive.return_value = str(Path(tempfile.gettempdir()) / "test_archive.zip")
        ...
        if res["status"] != "success": print("ERROR:", res)
        self.assertEqual(res["status"], "success")
        self.assertEqual(res["package_b64"], "BASE64_ZIP_DATA")
        
        mock_Extractor.assert_called_once()
        mock_ext.extract.assert_called_once()
        mock_ext.to_powershell.assert_called_once_with(mock_ext.to_powershell.call_args[0][0], enable_rollbacks=True)
    ...
    def test_handle_track_ckl(self, mock_Proc_class):
        ...
        mock_proc._extract_vuln_data.return_value = {
            "V-12345": {"status": "Open", "severity": "high", "finding_details": "Failed", "comments": "Fix"}
        }
    ...
    def test_handle_apply_results(self, mock_FixResPro_class, mock_encode):
        ...
        mock_proc.update_ckl.return_value = {"imported": 5, "updated": 0, "not_found": 0}
