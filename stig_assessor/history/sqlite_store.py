...
class SQLiteStore:
    ...
    def _init_db(self) -> None:
        ...
        with self._get_conn() as conn:
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_findings_vid ON findings(vid)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_findings_assess_id ON findings(assessment_id)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_assessments_asset ON assessments(asset_name, timestamp)')
            conn.commit()
            
    def save_assessment(self, asset_name: str, source_file: str, stig_title: str, results: List[Dict[str, Any]]) -> int:
        """
        ...
        with self._get_conn() as conn:
            ...
            cursor.executemany(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
                ''',
                [(assessment_id, r['vid'], r['status'], r['severity'], r.get('find', ''), r.get('comm', ''), r.get('chk', '')) 
                 for r in results]
            )
    ...
    def save_justification(self, vid: str, status: str, comments: str, who: str) -> None:
    ...
    def get_justification(self, vid: str) -> Optional[Dict[str, str]]:
        ...
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute('SELECT status, comments, updated_at, who FROM justifications WHERE vid = ?', (vid,))
            row = cursor.fetchone()
            if row:
                return dict(row)
            return None
    ...
    def get_drift(self, asset_name: str, current_assessment_id: int) -> Dict[str, Any]:
        ...
        with self._get_conn() as conn:
            ...
            cursor.execute('SELECT vid, status FROM findings WHERE assessment_id = ?', (prev_assessment_id,))
            prev_findings = {row['vid']: row['status'] for row in cursor.fetchall()}
            
            cursor.execute('SELECT vid, status FROM findings WHERE assessment_id = ?', (current_assessment_id,))
        ...
        for vid, curr_status in curr_findings.items():
            if vid not in prev_findings:
                ...
                elif prev_status.lower() in ("open", "not_reviewed") and curr_status.lower() == "notafinding":
                    drift["fixed"].append({"vid": vid, "from": prev_status, "to": curr_status})
                elif prev_status.lower() == "notafinding" and curr_status.lower() in ("open", "not_reviewed"):
                    drift["regressed"].append({"vid": vid, "from": prev_status, "to": curr_status})
                else:
                    drift["changed"].append({"vid": vid, "from": prev_status, "to": curr_status})
