...
def build_batch_tab(app, frame):
    ...
    ent_1 = ttk.Entry(io_frame, textvariable=app.batch_ind, width=GUI_ENTRY_WIDTH)
    ent_1.grid(row=0, column=1, padx=GUI_PADDING, sticky="we")

    def _browse_batch_in():
        path = filedialog.askdirectory(
            title="Select Input Directory", initialdir=app._last_dir()
        )
        if path:
    ...
    ent_2 = ttk.Entry(io_frame, textvariable=app.batch_out, width=GUI_ENTRY_WIDTH)
    ent_2.grid(
        row=1,
        column=1,
        padx=GUI_PADDING,
    ...
    def _browse_batch_out():
        path = filedialog.askdirectory(
            title="Select Output Directory", initialdir=app._last_dir()
        )
        if path:
    ...
    app._batch_tree = ttk.Treeview(
        results_frame, columns=tree_cols, show="headings", height=8
    )
    ...
    batch_scroll = ttk.Scrollbar(
        results_frame, orient="vertical", command=app._batch_tree.yview
    )
    ...
    def _do_batch_convert():
        ...
        files = (
            list(in_path.rglob("*.xml")) if recursive else list(in_path.glob("*.xml"))
        )
        if not files:
        ...
        def _process_chunk(idx):
            ...
            if idx >= len(files):
                ...
                app.status_var.set(
                    f"✔ Batch conversion complete. Success: {successes}, Failures: {failures}"
                )
                messagebox.showinfo("Batch Convert Complete", msg)
                if successes > 0 and messagebox.askyesno(
                    "Open Directory",
                    "Batch conversion complete. Would you like to open the output directory?",
                ):
                    import os
                    import subprocess
                    import sys
            ...
            for i in range(idx, chunk_end):
                fpath = files[i]
                asset_name = (
                    f"{prefix}_{fpath.stem.replace(' ', '_').replace('-', '_')}"
                )
                ...
                try:
                    ...
                    result = app.proc.xccdf_to_ckl(
                        fpath, out_file, asset_name, apply_boilerplate=bp
                    )
                    ...
                    if default_status != "Not_Reviewed":
                        ...
                        for v in tree.getroot().findall(".//VULN"):
                            ...
                            if s_node is not None:
                                s_node.text = default_status
                        FO.write_ckl(tree, out_file)

                    app._batch_tree.insert(
                        "",
                        tk.END,
                        values=(fpath.name, "Success", f"Processed {processed} rules"),
                        tags=("ok",),
                    )
                    successes += 1
                    ...
                    app._batch_tree.insert(
                        "",
                        tk.END,
                        values=(fpath.name, "Failed", str(e)),
                        tags=("error",),
                    )
                    failures += 1
            ...
            eta = (
                (elapsed / chunk_end) * (len(files) - chunk_end) if chunk_end > 0 else 0
            )
            app.status_var.set(
                f"Converting... {chunk_end}/{len(files)} (ETA: {int(eta)}s)"
            )
