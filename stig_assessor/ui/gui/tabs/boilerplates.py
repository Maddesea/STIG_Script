...
def build_boilerplates_tab(app, frame):
    ...
    ttk.Combobox(
        ...
        values=["All", Status.NOT_A_FINDING.value, Status.OPEN.value, Status.NOT_APPLICABLE.value, Status.NOT_REVIEWED.value],
        state="readonly",
        width=14,
    ).pack(side="left")
    ...
    def _bp_bulk_delete():
        ...
        deleted = 0
        for vid in selected:
            for status_val in [Status.NOT_A_FINDING.value, Status.OPEN.value,
                               Status.NOT_APPLICABLE.value, Status.NOT_REVIEWED.value]:
                if app.proc.boiler.delete(vid, status_val):
                    deleted += 1
        app.status_var.set(f"Deleted {deleted} boilerplate entries across {len(selected)} VIDs")
        _bp_refresh_vids()
    ...
    def _bp_import_from_ckl():
        ...
        status_filter = [filter_choice.strip()] if filter_choice and filter_choice.strip() else None
        ...
        try:
            ...
            _bp_refresh_vids()
            app.status_var.set(f"Imported {result['imported']} boilerplates from {path}")
            messagebox.showinfo(
                "Import Complete",
            ...
            messagebox.showerror("Import Error", f"Failed to import from checklist:\n{e}")
    ...
    def _bp_reset_single_vid():
        selected = list(app._bp_vids_list.selection())
        if not selected:
            messagebox.showinfo("No Selection", "Select VID(s) to reset to wildcard defaults.")
            return
        if not messagebox.askyesno(
            "Reset VID(s)",
    ...
    def _bp_find_duplicates():
        ...
        if not dupes:
            messagebox.showinfo("No Duplicates", "No duplicate boilerplate templates found.\n\nConsider using the V-* wildcard for common templates.")
            return
    ...
    ttk.Button(left_btn_frame, text="+ Add VID", command=_bp_add_vid).pack(fill="x", pady=1)
    ttk.Button(left_btn_frame, text="📥 Import from CKL", command=_bp_import_from_ckl).pack(fill="x", pady=1)
    ttk.Button(left_btn_frame, text="📋 Clone Selected", command=_bp_clone).pack(fill="x", pady=1)
    ttk.Button(left_btn_frame, text="↩ Reset VID to Wildcard", command=_bp_reset_single_vid).pack(fill="x", pady=1)
    ttk.Button(left_btn_frame, text="🔎 Find Duplicates", command=_bp_find_duplicates).pack(fill="x", pady=1)
    ttk.Button(left_btn_frame, text="🗑 Bulk Delete", command=_bp_bulk_delete).pack(fill="x", pady=1)
    ttk.Button(left_btn_frame, text="🔄 Reset All Defaults", command=_bp_reset_defaults).pack(fill="x", pady=1)
    ...
    apply_frame = ttk.LabelFrame(right_frame, text="Apply Boilerplates to Target Checklist", padding=GUI_PADDING_LARGE)
    ...
    def _pull_selected_vids():
        ...
        if not sel:
            messagebox.showinfo("No VIDs Selected", "Please select one or more VIDs from the left panel list.")
            return
        app._bp_apply_vids_var.set(", ".join(sel))

    ttk.Button(apply_ctrls_2, text="← Use Selected", command=_pull_selected_vids, width=14).pack(side="left", padx=(0, 15))
    ...
    ToolTip(date_ent, "Override {date} variable in templates (format: YYYY-MM-DD). Leave as current or change for retroactive assessment dates.")
    ...
    ttk.Button(apply_ctrls_2, text="📅 Today", command=_reset_date_to_today, width=8).pack(side="left", padx=(0, 15))
    ...
    def _bp_apply_to_ckl(dry_run=False):
        ...
        def done(res):
            ...
            if isinstance(res, Exception):
                messagebox.showerror("Apply Error", f"Failed to apply boilerplates:\n{res}")
                return
            ...
            if dry_run:
                app.status_var.set(f"Preview: {res['updated']} items would be updated in {Path(path).name}")
            else:
                app.status_var.set(f"Applied boilerplates to {res['updated']} items in {Path(save_path).name}")
                messagebox.showinfo(
                    "Apply Complete",
    ...
    ttk.Button(
        apply_btn_frame, text="👁 Preview Apply...", command=lambda: _bp_apply_to_ckl(dry_run=True), width=18
    ).pack(side="left", padx=(0, 10))

    ttk.Button(
        apply_btn_frame, text="⚡ Select CKL & Apply...", command=lambda: _bp_apply_to_ckl(dry_run=False), style="Accent.TButton"
    ).pack(side="left")

    # Preview Treeview
    app._bp_apply_tree = ttk.Treeview(apply_frame, columns=("vid", "action"), show="headings", height=4)
    ...
    tree_scroll = ttk.Scrollbar(app._bp_apply_tree, orient="vertical", command=app._bp_apply_tree.yview)
    app._bp_apply_tree.configure(yscrollcommand=tree_scroll.set)
    tree_scroll.pack(side="right", fill="y")

    # Template variable picker
    ttk.Label(ctrl_frame, text="  Insert:").pack(side="left", padx=(GUI_PADDING_LARGE, 2))
    ...
    # Preview label
    preview_frame = ttk.LabelFrame(editors, text="Preview (with variables substituted)", padding=4)
    preview_frame.grid(row=4, column=0, sticky="ew", pady=(GUI_PADDING, 0))
    app._bp_preview_var = tk.StringVar(value="")
    ttk.Label(preview_frame, textvariable=app._bp_preview_var, wraplength=600, foreground=app._colors.get("info", "blue")).pack(fill="x")

    def _update_preview(*args):
        finding = app._bp_finding_text.get("1.0", "end-1c")[:200]
        if finding.strip():
            preview = finding.replace("{asset}", "WEBSERVER01").replace("{severity}", "high")
            date_val = app._bp_apply_date_var.get().strip() or datetime.now().strftime("%Y-%m-%d")
            preview = preview.replace("{date}", date_val)
            preview = preview.replace("{timestamp}", datetime.now().strftime("%Y-%m-%d"))
            preview = preview.replace("{vid}", app._bp_current_vid or "V-XXXXX")
            preview = preview.replace("{status}", app._bp_status_var.get())
            app._bp_preview_var.set(f"Preview: {preview[:180]}…" if len(preview) > 180 else f"Preview: {preview}")
        else:
    ...
    def _bp_import_json():
        ...
        try:
            ...
            messagebox.showinfo("Import Successful", f"Imported boilerplates from:\n{path}")
            _bp_refresh_vids()
        except Exception as e:
