...
from stig_assessor.core.constants import (GUI_BUTTON_WIDTH_WIDE,
                                          GUI_ENTRY_WIDTH, GUI_LISTBOX_HEIGHT,
...
def build_merge_tab(app, frame):
    ...
    ttk.Button(input_frame, text="📂 Browse…", command=_browse_merge_base).grid(
        row=0, column=2
    )
    ...
    ttk.Label(
        input_frame,
        text="(💡 Drag & drop supported)",
        font=("TkDefaultFont", 8),
        foreground="gray",
    ).grid(row=2, column=0, sticky="nw", padx=(0, 4))
    ...
    ttk.Entry(out_frame, textvariable=app.merge_out, width=GUI_ENTRY_WIDTH).grid(
    ...
    ttk.Button(out_frame, text="📂 Browse…", command=_browse_merge_out).grid(
        row=0, column=2
    )
    ...
    ttk.Button(out_frame, text="🗑 Clear Form", command=_clear_merge_form).grid(
    ...
    granular_frame = ttk.LabelFrame(
        frame, text="Merge Strategy & Granular Control", padding=GUI_PADDING_LARGE
    )
    ...
    ttk.Label(
        strategy_row, text="Conflict Resolution:", font=("TkDefaultFont", 10, "bold")
    ...
    ttk.Label(filter_row, text="Filter by Status (comma-sep, optional):").pack(
        side="left", padx=(0, 4)
    )
    app.merge_status_filter = tk.StringVar(value="")
    ttk.Entry(filter_row, textvariable=app.merge_status_filter, width=20).pack(
        side="left", padx=(0, GUI_PADDING_LARGE)
    )

    ttk.Label(filter_row, text="Filter by Severity (comma-sep, optional):").pack(
        side="left", padx=(0, 4)
    )
    app.merge_severity_filter = tk.StringVar(value="")
    ttk.Entry(filter_row, textvariable=app.merge_severity_filter, width=20).pack(
        side="left"
    )
    ...
    def _load_merge_profile():
        ...
        try:
            with open(path, "r", encoding="utf-8") as f:
                profile = json.load(f)
            if "preserve_history" in profile:
                app.merge_preserve.set(profile["preserve_history"])
            if "apply_boilerplates" in profile:
                app.merge_bp.set(profile["apply_boilerplates"])
            if "conflict" in profile:
                app.merge_conflict.set(profile["conflict"])
            if "details_mode" in profile:
                app.merge_details_mode.set(profile["details_mode"])
            if "comments_mode" in profile:
                app.merge_comments_mode.set(profile["comments_mode"])
            if "status_mode" in profile:
                app.merge_status_mode.set(profile["status_mode"])
            if "status_filter" in profile:
                app.merge_status_filter.set(profile["status_filter"])
            if "severity_filter" in profile:
                app.merge_severity_filter.set(profile["severity_filter"])
            app.status_var.set("Merge profile loaded successfully.")
        except Exception as e:
            messagebox.showerror("Load Error", str(e))

    ttk.Button(profile_row, text="💾 Save Profile", command=_save_merge_profile).pack(
        side="left", padx=(0, 10)
    )
    ttk.Button(profile_row, text="📂 Load Profile", command=_load_merge_profile).pack(
        side="left"
    )
    ...
    def _build_merge_kwargs():
        ...
        status_f = (
            [s.strip() for s in app.merge_status_filter.get().split(",")]
            if app.merge_status_filter.get()
            else None
        )
        sev_f = (
            [s.strip() for s in app.merge_severity_filter.get().split(",")]
            if app.merge_severity_filter.get()
            else None
        )
    ...
    preview_frame = ttk.LabelFrame(
        frame, text="Affected Rules", padding=GUI_PADDING_LARGE
    )
    ...
    preview_scroll = ttk.Scrollbar(
        preview_frame, orient="vertical", command=app.merge_preview_tree.yview
    )
    ...
    ttk.Button(tree_toolbar, text="Select All", command=_select_all_merge).pack(
        side="left", padx=(0, 5)
    )
    ttk.Button(
        tree_toolbar, text="Clear Selection", command=_clear_merge_selection
    ...
    def _do_preview():
        if not app.merge_base.get() or not app.merge_histories:
            messagebox.showinfo(
                "Missing Input", "Base and History files are required for preview."
            )
            return
        ...
        def done(res):
            ...
            for p in preview_list:
                ...
                for c in p.get("changes", []):
                    if c["field"] == "status":
                        ...
                        change_strs.append(
                            f"{c['field']} length: {c['from_length']}->{c['to_length']}"
                        )
                app.merge_preview_tree.insert(
                    "",
                    tk.END,
                    values=(
                        p.get("vid", ""),
            ...
            app.status_var.set(
                f"Preview: {res.get('total_affected', 0)} VIDs would be affected ({res.get('filtered', 0)} filtered)"
            )
    ...
    def _do_merge(dry=False):
        if not app.merge_base.get() or not app.merge_histories:
            app._show_inline_error(
                btn_merge, "Missing input: base and history required."
            )
            return
        ...
        out_path_str = (
            app.merge_out.get()
            if not dry
            else app.merge_base.get().replace(".ckl", "_preview.ckl")
        )
        if not dry and Path(out_path_str).exists():
            if not messagebox.askyesno(
                "Overwrite?", f"{Path(out_path_str).name} already exists. Overwrite?"
            ):
                return
        ...
        if app.merge_selected_only.get():
            ...
            if not sel_items:
                messagebox.showwarning(
                    "No Selection",
                    "You checked 'Only merge selected' but selected nothing in the preview grid.",
                )
                return

            vid_list = [
                app.merge_preview_tree.item(item, "values")[0] for item in sel_items
            ]
        ...
        def done(result):
            ...
            if dry:
                _do_preview()
                app.status_var.set(
                    f"Dry Run: {updated} would change, {skipped} skipped, {filtered} filtered"
                )
            else:
    ...
    ToolTip(
        btn_preview,
        "Run a dry-run merge to preview what would change\nwithout writing any files.",
    )
    ...
    # ═══ Merge Results Panel ═══
    app.merge_results_frame = ttk.LabelFrame(
        frame, text="Merge Results Summary", padding=GUI_PADDING_LARGE
    )
    ...
    ttk.Label(
        stats_row,
        textvariable=app.merge_updated_var,
        foreground="#10b981",
        font=("TkDefaultFont", 10, "bold"),
    ).pack(side="left", padx=(0, GUI_PADDING_LARGE))
    ttk.Label(
        stats_row,
        textvariable=app.merge_skipped_var,
        foreground="#6b7280",
        font=("TkDefaultFont", 10, "bold"),
    ).pack(side="left", padx=GUI_PADDING_LARGE)
    ttk.Label(
        stats_row,
        textvariable=app.merge_protected_var,
        foreground="#f59e0b",
        font=("TkDefaultFont", 10, "bold"),
    ).pack(side="left", padx=GUI_PADDING_LARGE)
