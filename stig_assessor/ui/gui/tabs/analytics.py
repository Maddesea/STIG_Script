...
from stig_assessor.core.constants import GUI_PADDING, GUI_PADDING_LARGE, GUI_ENTRY_WIDTH, GUI_PADDING_SECTION, GUI_BUTTON_WIDTH_WIDE, GUI_FONT_MONO, GUI_FONT_NORMAL, Status
...
def build_analytics_tab(app, frame):
    ...
    ttk.Label(header_frame, text="View Analysis for:", font=GUI_FONT_NORMAL).pack(side="left")
    ...
    tk.Radiobutton(header_frame, text="Single Checklist", variable=app.analytics_mode, value="single", command=_on_mode_change).pack(side="left", padx=GUI_PADDING_LARGE)
    tk.Radiobutton(header_frame, text="Fleet / Enclave", variable=app.analytics_mode, value="fleet", command=_on_mode_change).pack(side="left", padx=GUI_PADDING)

    # ── SINGLE CHECKLIST CONTROLS ──
    single_ctrl_frame = ttk.LabelFrame(frame, text="Single Checklist Analysis", padding=GUI_PADDING_LARGE)
    ...
    ent_1 = ttk.Entry(single_ctrl_frame, textvariable=app.stats_ckl, width=GUI_ENTRY_WIDTH)
    ent_1.grid(row=0, column=1, padx=GUI_PADDING, sticky="we")

    def _browse_stats_ckl():
        app.stats_ckl.set(filedialog.askopenfilename(initialdir=app._last_dir(), filetypes=[("CKL", "*.ckl")]))
    ttk.Button(single_ctrl_frame, text="📂 Browse…", command=_browse_stats_ckl).grid(row=0, column=2)
    app._enable_dnd(ent_1, app.stats_ckl)

    # ── FLEET / ENCLAVE CONTROLS ──
    fleet_ctrl_frame = ttk.LabelFrame(frame, text="Fleet / Enclave Analytics", padding=GUI_PADDING_LARGE)
    fleet_ctrl_frame.columnconfigure(1, weight=1)

    ttk.Label(fleet_ctrl_frame, text="Enclave Dir/ZIP:").grid(row=0, column=0, sticky="w")
    app.fleet_source = tk.StringVar()
    ent_fleet = ttk.Entry(fleet_ctrl_frame, textvariable=app.fleet_source, width=GUI_ENTRY_WIDTH)
    ent_fleet.grid(row=0, column=1, padx=GUI_PADDING, sticky="we")

    def _browse_fleet():
        path = filedialog.askdirectory(title="Select Enclave Directory")
        if not path:
            path = filedialog.askopenfilename(title="Select Enclave ZIP", filetypes=[("ZIP Archive", "*.zip")])
        if path: app.fleet_source.set(path)
    
    ttk.Button(fleet_ctrl_frame, text="📂 Browse…", command=_browse_fleet).grid(row=0, column=2)
    ...
    def _do_stats_tab():
        ...
        def work():
            if mode == "single":
                ...
                if len(res.get("asset_compliance", [])) > 20:
                    summary.append(f"... and {len(res['asset_compliance']) - 20} more assets.")
        ...
        def done(result):
            ...
            if total == 0:
                app.stats_canvas.create_text(300, 110, text="No vulnerabilities found.", fill="gray", font=GUI_FONT_NORMAL)
                return
            ...
            title_txt = f"Compliance Posture: {source[:40]}..." if mode == "single" else f"Enclave Performance ({stats_dict.get('total_assets', 0)} Assets)"
            app.stats_canvas.create_text(width / 2, 20, text=title_txt, fill=app._colors.get("fg", "#374151"), font=(GUI_FONT_NORMAL[0], 12, "bold"))

            by_status = stats_dict.get("by_status", {})
            bars = [Status.NOT_A_FINDING.value, Status.OPEN.value, Status.NOT_APPLICABLE.value, Status.NOT_REVIEWED.value]
            ...
            for i, k in enumerate(bars):
                ...
                PremiumChart.draw_bar(
                    app.stats_canvas, x_pos, height - 35, bar_w, h,
                    colors.get(k, "#3b82f6"), labels[i], count,
    ...
    def _clear_analytics_form():
        ...
        app.stats_results_txt.configure(state="disabled")
        app.stats_canvas.delete("all")
        app.stats_canvas.create_text(300, 110, text="Load data to view graphical compliance dashboard", fill="gray", font=GUI_FONT_NORMAL)
    ...
    btn = ttk.Button(btn_row, text="📊 Run Analytics", command=_do_stats_tab, width=GUI_BUTTON_WIDTH_WIDE, style="Accent.TButton")
    btn.pack(side="left", padx=GUI_PADDING)
    ttk.Button(btn_row, text="💾 Export Report", command=_export_analytics).pack(side="left", padx=GUI_PADDING)
    ttk.Button(btn_row, text="🗑 Clear Form", command=_clear_analytics_form).pack(side="left", padx=GUI_PADDING)

    app.stats_canvas = tk.Canvas(frame, height=220, bg=app._colors.get("bg", "#ffffff"), highlightthickness=0)
